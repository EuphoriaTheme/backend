...
const MAX_TRANSLATION_ITEMS = Number.parseInt(process.env.MAX_TRANSLATION_ITEMS || '250', 10);
const MAX_TRANSLATION_TEXT_LENGTH = Number.parseInt(process.env.MAX_TRANSLATION_TEXT_LENGTH || '256', 10);
const LANGUAGE_LIST_CACHE_TTL_MS = Number.parseInt(process.env.LANGUAGE_LIST_CACHE_TTL_MS || '30000', 10);
...
function getCachedTranslationMap(targetLang) {
  ...
  if (!fs.existsSync(filePath)) {
    return { error: `Translations for language "${targetLang}" are not available.`, statusCode: 400 };
  }
...
export default async function registerTranslationsRoutes(app) {
  app.post('/translate/bulk', async (request, reply) => {
    ...
    if (!texts || !Array.isArray(texts) || texts.length === 0 || !targetLang) {
      return reply.code(400).send({ success: false, error: 'Texts and target language are required.' });
    }

    if (!isValidLanguageCode(targetLang)) {
      return reply.code(400).send({ success: false, error: 'targetLang contains invalid characters.' });
    }

    if (texts.length > MAX_TRANSLATION_ITEMS) {
      return reply.code(413).send({ success: false, error: `Maximum ${MAX_TRANSLATION_ITEMS} texts per request.` });
    }

    for (const text of texts) {
      if (typeof text !== 'string') {
        return reply.code(400).send({ success: false, error: 'Each text item must be a string.' });
      }

      if (text.length > MAX_TRANSLATION_TEXT_LENGTH) {
        return reply.code(413).send({ success: false, error: `Each text item must be <= ${MAX_TRANSLATION_TEXT_LENGTH} characters.` });
      }
    }
    ...
    try {
      ...
      return reply.code(500).send({ success: false, error: 'Failed to translate texts.' });
    }
  });
