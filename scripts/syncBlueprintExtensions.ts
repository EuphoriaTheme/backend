...
const BLUEPRINT_FILE = path.resolve(
  __dirname,
  "..",
  "public",
  "blueprint.json",
...
const BLUEPRINT_SYNC_USER_AGENT =
  process.env.BLUEPRINT_SYNC_USER_AGENT ||
  "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/141.0.0.0 Safari/537.36";
const BLUEPRINT_SYNC_CF_CLEARANCE =
  process.env.BLUEPRINT_SYNC_CF_CLEARANCE || "";
const BLUEPRINT_SYNC_COOKIE = process.env.BLUEPRINT_SYNC_COOKIE || "";
const BLUEPRINT_SYNC_ORIGIN =
  process.env.BLUEPRINT_SYNC_ORIGIN || "https://blueprintframe.work";
const BLUEPRINT_SYNC_REFERER =
  process.env.BLUEPRINT_SYNC_REFERER || "https://blueprintframe.work/";
const BLUEPRINT_SYNC_TIMEOUT_MS =
  Number(process.env.BLUEPRINT_SYNC_TIMEOUT_MS) || 20000;
...
export async function syncBlueprintExtensions() {
  const urlsToTry = [
    BLUEPRINT_EXTENSIONS_URL,
    BLUEPRINT_EXTENSIONS_FALLBACK_URL,
  ].filter(Boolean);
  ...
  try {
    ...
    for (const url of urlsToTry) {
      try {
        ...
        const isCloudflareChallenge =
          String(
            error?.response?.headers?.["cf-mitigated"] || "",
    ...
    if (!isJsonPayload) {
      console.error(
        "[blueprint-sync] Response was not JSON; skipped writing blueprint.json",
      );
      return false;
    }
    ...
    console.log(
      `[blueprint-sync] Updated ${BLUEPRINT_FILE} from ${sourceUrl} at ${new Date().toISOString()}`,
    );
    return true;
  } catch (error) {
    const message = error?.response
      ...
      : error.message;
    const isCloudflareChallenge =
      String(error?.response?.headers?.["cf-mitigated"] || "").toLowerCase() ===
      "challenge";
    if (isCloudflareChallenge) {
      console.error(
        "[blueprint-sync] Failed to update blueprint.json: blocked by Cloudflare challenge (set BLUEPRINT_SYNC_COOKIE from a browser session, or use a whitelisted/tokenized API endpoint).",
      );
    } else {
      console.error(
        `[blueprint-sync] Failed to update blueprint.json: ${message}`,
      );
    }
    return false;
  }
