...
const LICENSE_API_V1_URL =
  process.env.LICENSE_API_V1_URL ||
  "https://license.euphoriadevelopment.uk/api/v1/validate";
const LICENSE_API_V2_URL =
  process.env.LICENSE_API_V2_URL ||
  "https://licensing.euphoriadevelopment.uk/api/licenses/validate";
const LICENSE_API_V2_TOKEN = String(
  process.env.LICENSE_API_V2_TOKEN || "",
).trim();
const LICENSE_API_TIMEOUT_MS = Number.parseInt(
  process.env.LICENSE_API_TIMEOUT_MS || "10000",
  10,
);
const LICENSE_API_MAX_RETRIES = Number.parseInt(
  process.env.LICENSE_API_MAX_RETRIES || "1",
  10,
);
...
export async function validateLicense({
  ...
  const endpoint =
    normalizedVersion === "v2" ? LICENSE_API_V2_URL : LICENSE_API_V1_URL;
